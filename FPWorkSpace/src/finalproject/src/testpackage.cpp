...
int main(int argc, char **argv) {
  ...
  Navigator navigator(
      true); // create node with no debug info and no verbose messages
  ...
  while (!navigator.IsTaskComplete()) {          // test IsTaskComplete
    ...
    auto ptr_spin =
        std::static_pointer_cast<const nav2_msgs::action::Spin::Feedback>(
            feedback_ptr);
  ...
  while ((!navigator.IsTaskComplete()) &&
         (i < 3)) { // wait for 3 feedback messages and then cancel
    i++;
  }
